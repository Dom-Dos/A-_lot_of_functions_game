...
def haendler_upgrade(goldbeutel, spielerHP, spielerMaxHP, spielerStr, spielerVer, mana, maxmana, trank):
    ...
    waren = {
        "1": {"Name": "Oger-Protein", "Preis": 20, "Effekt": "Str", "Wert": 3, "Info": "+3 Stärke"},
        ...
        "4": {"Name": "Lebenselixier", "Preis": 30, "Effekt": "HP", "Wert": 20, "Info": "+20 Max HP"},
    ...
    while True:
        ...
        wahl = input("\nWas möchtest du kaufen? ").strip() # .strip() entfernt Leerzeichen
        ...
        if wahl in waren:
            ...
            if goldbeutel >= item["Preis"]:
                ...
                if item["Effekt"] == "Str":
                    ...
                    maxmana += item["Wert"]
                    mana = maxmana # Mana voll auffüllen
                elif item["Effekt"] == "HP":
                    spielerMaxHP += item["Wert"]
                    spielerHP = spielerMaxHP # HP voll auffüllen
                elif item["Effekt"] == "Trank":
                    trank += item["Wert"]
                
                print(f"\nErfolgreich gekauft! {item['Name']}.")
            else:
...
def torwaechter_raetsel(spielerHP):
    ...
    while versuche > 0:
        ...
        antwort = input("1) Herscht hier ein Diktator (ja/nein) ").lower().strip()
        if antwort == "ja":
            punkte += 1

        antwort = input("2) Ist der Mars größer als der Merkur (ja/nein) ").lower().strip()
        if antwort == "ja":
            punkte += 1

        antwort = input("3) Gibt es hier Demokratie (ja/nein) ").lower().strip()
        if antwort == "nein":
            punkte += 1
...
def optionen(Krieger,Magier,Schütze,Assasine,trank,Mana,MaxMana,DMG,SpielerHP,SpielerMaxHP,SpielerRüstung,GegnerHP,GegnerRüstung,GegnerDMG,Giftdauer,Giftschaden,blocken,schutz,gefroren,Sneakschaden,ausweichen,nextAtt,runde,win):
    ...
    while auswahl == True:
        if Krieger == 1:
            ...
            ant = input('Du willst 1 , 2, 3, 4 oder  ?\n').strip()
        ...
        if Magier == 1:
            print('Das sind deine Optionen:')
            print('option 1: Angriff')
            print('option 2: Feuerball Mana Kosten 3')
            print('option 3: RüstungMinus Mana Kosten 7')
            print('option 4: Eis Mana Kosten 5 ')
            print('option 5: Heiltrank Stück;',trank)
            ant = input('Du willst 1 , 2 ,3 , 4, 5 ?\n').strip()
            Krit = random.randint(0,3)
            if ant == "1":
                schadengegner,nextAtt = Schaden(DMG,Krit,GegnerRüstung,nextAtt)
                GegnerHP = GegnerHP - schadengegner
...
while bestaetigung == False:
    option = input('Welche Klasse soll es sein?\n 1 ,2 ,3 oder 4?\n').strip()
    if option == "1" :
...
ant = input('1 = ja| 2 = nein\n').lower()
platz(1)
karma = 0
if ant == '1' and goldbeutel >= 1 or ant == 'ja' and goldbeutel >= 1:
    print('Der Bettler freut sich und bedankt sich vielmals')
    print('Eventuell bringt dir das Gute Karma Später etwas')
    goldbeutel -= 1
    karma += 1
elif ant == '1' and goldbeutel <1 or ant == 'ja' and goldbeutel <1:
    print('Du kannst den Bettler leider nichts geben, Du hast keine Münzen mehr')    
elif ant == '2' and goldbeutel >= 1 or ant == 'nein' and goldbeutel >= 1:
...
while True and spielerHP >= 0:
    ...
    richtung = input("Dein Zug: ").lower().strip()
...
while True and spielerHP >= 0:
    ...
    raum_betreten(spieler_pos, besucht2)   
